...
- `Error`: Error returned by server (if any)
  - For failed requests contains detailed error message returned by CosmosDB REST API as JSON string
- `Details`: Detailed information provided by server
  - `ActivityId`: Id of request generated by server or mirrorred from request
  ...
  - `ResourceUsage`: Information about document(s) and collection
